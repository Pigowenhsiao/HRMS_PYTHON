Public Class CF_Check_OverDue
    ...
    Sub GetData()
        ...
        Call mDB_Link()
        strSQL = ""
        strSQL = "select A.EMP_ID as 工號, C_NAME as 姓名,A.Certify_ID as 認證ID,Certify_Name as 認證名稱,"
        ...
        strSQL &= " and A.active =true and B.active=true and C.active=true "
    ...
    Private Sub BT_CF_OverDue_Special_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles BT_CF_OverDue_Special.Click
        ...
        strSQL &= " Basic  C "
        strSQL &= " where (C.EMP_ID = A.EMP_ID And Trim(B.Certify_ID) = Trim(A.Certify_ID)) "
        strSQL &= " and B.active=true and C.active=true and DateDiff(now(),certify_date) >=" & Trim(TB_CF_OverDue_Secial.Text) & ""
